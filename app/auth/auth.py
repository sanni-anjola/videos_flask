...
# @roles_required(UserRole.SUPER_USER.value)
@jwt_required()
def add_role(id: str):
    roles = request.get_json()  # Access JSON data from request
    
    users.find_one_and_update({"_id": ObjectId(id)}, {"$push": {"roles": roles['role']}})
    
    # Return the inserted movie data as JSON
    return "Role updated successfully", 200